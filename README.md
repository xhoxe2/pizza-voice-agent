...
## Безпека та продакшн-нотатки

Модель загроз для тестового завдання й те, що варто додати на проді:

- **Доступ.** Агент не є відкритим ендпоінтом: щоб з ним поговорити, клієнт
  має приєднатися до LiveKit-кімнати за токеном, підписаним
  `LIVEKIT_API_SECRET`. Це базовий бар'єр від сторонніх.
- **Секрети** лише в `.env`, у контекст моделі не потрапляють — промпт-ін'єкція
  не може їх дістати. Інструменти не виконують довільний код/SQL/shell.
- **Валідація.** Аргументи інструментів від моделі перевіряються через pydantic
  (`OrderLine`), некоректний ввід не валить агента.
- **На проді додати:** rate-limit і ліміт тривалості сесії (захист від
  навмисного «спалювання» токенів), стелю витрат на стороні OpenAI,
  прив'язку запиту статусу замовлення до авторизованого клієнта
  (зараз `get_order_status` не повертає ПДн, але номер перелічуваний),
  моніторинг і алерти.
